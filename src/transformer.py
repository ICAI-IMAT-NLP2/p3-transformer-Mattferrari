...
class Transformer(nn.Module):
    ...
    def __init__(self, src_vocab_size: int, tgt_vocab_size: int, max_enc_position_embeddings: int, max_dec_position_embeddings: int,
        ...
        self.encoder = TransformerEncoder(vocab_size=src_vocab_size, 
        ...
        self.decoder = TransformerDecoder(vocab_size=tgt_vocab_size, 
                                          ...
                                          num_attention_heads=num_attention_heads)
        self.output_linear = nn.Linear(in_features=dec_d_model, 
    ...
    def __greedy_decode(self, src_input: torch.Tensor, max_length: int, **kwargs) -> torch.Tensor:
        ...
        tgt_input = torch.full((batch_size, 1), SOS_token, device=device, dtype=torch.long)
        ...
        for _ in range(max_length):
            ...
            next_token = probs.argmax(dim=-1, keepdim=True)  # Shape: (batch_size, 1)
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                for next_token in range(log_probs.size(1)):
                    next_log_prob = log_probs[0, next_token].item()
    ...
    def __sampling_decode(self, src_input: torch.Tensor, max_length: int, temperature: float = 1.0, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            scaled_logits = torch.log(torch.softmax(logits, dim=-1))/temperature

            # Apply softmax to get probabilities
            probs = torch.softmax(scaled_logits, dim=-1)

            # Sample from the probability distribution
            next_token = torch.multinomial(probs, 1)  # Shape: (batch_size, 1)
    ...
    def __top_k_sampling_decode(self, src_input: torch.Tensor, max_length: int, k: int = 10, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            probs = torch.multinomial(torch.softmax(topk_log_probs, dim=-1), num_samples=1)
            next_token = topk_indices[torch.arange(batch_size), probs.squeeze()]  # Shape: (batch_size, 1)
    ...
    def __top_p_sampling_decode(self, src_input: torch.Tensor, max_length: int, p: float = 0.9, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            sorted_indices_to_remove = sorted_indices[cumulative_probs > p]
            sorted_probs[:, sorted_indices_to_remove] = 0
            # Normalize the probabilities
            sorted_probs = sorted_probs/torch.sum(sorted_probs, dim=-1).unsqueeze(1)
            # Sample from the filtered distribution
            next_token = torch.multinomial(sorted_probs, num_samples=1)
            # Map sampled indices to original token indices
            next_token = sorted_indices[torch.arange(batch_size), next_token.squeeze()]
            # Append next token to tgt_input
            tgt_input = torch.cat((tgt_input, next_token.unsqueeze(1)), dim=-1)
            # Check if all sequences have generated EOS_token
            if (next_token == EOS_token).all():
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            expanded_tgt_input = torch.expand_copy(tgt_input, (batch_size, k, tgt_input.shape[1]))  # Shape: (k, seq_len)
            ...
            dec_outputs_candidate = []
            for i in range(k):
                out = self.decoder(y_candidates[:, i, :], enc_output) 
                dec_outputs_candidate.append(out)
            dec_outputs_candidate = torch.stack(dec_outputs_candidate, dim=1)  # (batch_size, k, seq_len+1, hidden_size)
            ...
            h_v_norm = h_v / (h_v.norm(dim=-1, keepdim=True) + 1e-8)  # Shape: (k, hidden_size)
            h_j_norm = h_j / (h_j.norm(dim=-1, keepdim=True) + 1e-8)  # Shape: (k, seq_len, hidden_size)

            # Compute cosine similarities between h_v and each h_j
            cos_sim = (h_v_norm.unsqueeze(-2)*h_j_norm).sum(dim=-1)  # Shape: (k, seq_len)

            # Get maximum cosine similarity for each candidate
            max_sim, _ = cos_sim.max(-1)  # Shape: (k,)
